...
pair<int, vector<int>> bfsTSP(const vector<vector<int>>& dist) {
    ...
    //Începem din fiecare oraș și inițializam coada și tabelul minCost
    for (int i = 0; i < n; ++i) {
        q.push({ 1 << i, i, 0, 0, {i} });
        minCost[1 << i][i] = 0;
    }
    ...
    while (!q.empty()) {
        ...
        for (int nextCity = 0; nextCity < n; ++nextCity) {
            if (!(current.mask & (1 << nextCity))) { //Conditie if daca orasul nu a fost vizitat
...
pair<int, vector<int>> ucsTSP(const vector<vector<int>>& dist) {
    ...
    // Începem din fiecare oraș și inițializam priority queue si minCost table
    for (int i = 0; i < n; ++i) {
        pq.push({ 1 << i, i, 0, 0, {i} });
        minCost[1 << i][i] = 0;
    }
